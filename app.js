...
function compareWords(){
    let yellow = []; //yellow char array
    let green = []; //green char array
    let matches = 0;
    for(let i = 0; i < 5; i++){
        used.push(curr[i]);
         //character matches (green case)
        if(curr[i] == solution[i]){
            ...
            green.push(curr[i]);
            if(yellow.includes(curr[i])){ //if formerly yellow, remove from array
                yellow.splice(yellow.indexOf(curr[i]),1);
            }
            matches++;

        //character present in solution (yellow case)
        }else if(solution.includes(curr[i])){ 
            let occurences = solution.split(curr[i]).length - 1; //find # of occurences of letter in solution
            let found = 0;
            ...
            if(green.includes(curr[i])){ //if at least one char already matched
                if(occurences == found){
        ...
        //character not in solution (grey case)
        }else{ 
            document.getElementById(row + "," + i).style.backgroundColor = "rgb(52, 52, 52)";
            document.getElementById(row + "," + i).style.outlineColor = "rgb(52, 52, 52)";
